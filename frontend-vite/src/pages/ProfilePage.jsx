...
const ProfilePage = () => {
  ...
  return (
    ...
    <div className="z-10 max-w-xl w-full bg-white/20 backdrop-blur-lg rounded-3xl shadow-2xl p-8 relative border border-white/20 flex flex-col items-center text-center gap-6">
      <h1 className="text-3xl font-bold sm:text-2xl xs:text-xl">Hey {profileName} 👋</h1>
  
      {profilePic && (
        <img
          src={profilePic}
          alt="Profile"
          className="w-28 h-28 rounded-full border-4 border-white shadow-lg sm:w-24 sm:h-24 xs:w-20 xs:h-20"
        />
      )}
  
      <p className="text-lg font-semibold text-white bg-white/10 px-4 py-2 rounded-full shadow-inner sm:text-base xs:text-sm">
        @{username}
      </p>
